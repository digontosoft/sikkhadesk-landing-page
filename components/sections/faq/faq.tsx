...
function Faq() {
  return (
    <Section id="faq" className="relative overflow-hidden">
      ...
      >
        <MessageCircleQuestion className="text-brand-blue size-28" />
      </div>
      <div
        aria-hidden
        className="pointer-events-none absolute top-32 left-24 -z-10 hidden opacity-10 md:left-36 lg:block"
      >
        <MessageCircleWarning className="text-brand-blue size-14" />
      </div>
      <div
        aria-hidden
      ...
      >
        <Badge>
          <HelpCircle className="fill-brand-emerald/20 text-brand-emerald size-3.5" />
          সাধারণ প্রশ্নাবলী (FAQ)
        </Badge>
        <Heading size="lg" className="max-w-2xl text-balance">
          আপনার যেকোনো <span className="text-gradient-hero">প্রশ্নের উত্তর</span> এখানে
        </Heading>
        <p className="max-w-2xl text-balance text-muted-foreground sm:text-lg">
          SikkhaDesk সম্পর্কে আমাদের গ্রাহকদের করা কিছু সাধারণ প্রশ্ন ও তার উত্তর দেওয়া হলো। তাও না
          পেলে, আমাদের সাপোর্ট টিমের সাথে যোগাযোগ করুন।
        </p>
      ...
      <div className="mt-12 grid gap-6 lg:grid-cols-2">
        {faqCategories.map((category, index) => (
          <FaqCategoryCard key={category.title} category={category} index={index} />
        ))}
      </div>
      ...
      >
        <div className="flex flex-col items-center gap-4 text-center sm:flex-row sm:text-left">
          <span className="relative flex size-16 shrink-0 items-center justify-center">
            ...
            />
            <div className="ring-primary/20 relative size-16 overflow-hidden rounded-full ring-2">
              <Image
